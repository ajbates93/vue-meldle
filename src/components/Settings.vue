  ...
  <div class="z-50 w-2/4 fixed p-5 top-2/4 left-2/4 bg-white dark:bg-slate-800 shadow-xl rounded-lg transition-all" style="transform: translate(-50%, -50%); max-width: 350px; min-width: 300px;" :class="store.state.settings.showSettings ? 'opacity-100 visible' : 'opacity-0 hidden'">
    <div class="relative">
      ...
      <div class="stats-values grid gap-2 grid-cols-4 dark:text-white">
        ...
        <div class="item">
          <span class="value block text-center text-2xl font-bold text-green-400" v-text="formattedWinPercentage"></span>
      ...
      <div class="stats-charts my-5">
        <h5 class="font-bold mb-3 dark:text-white">Average Successful Guesses</h5>
        <div class="chart flex my-1 items-center" v-for="(guess, idx) in guesses" :key="idx">
          <div class="chart-label mr-3 font-bold dark:text-white">{{idx}}</div>
...
const formattedWinPercentage = computed(() => {
  let win = store.state.stats.winPercentage
  return (Math.round(win * 100) / 100).toFixed(2)
})
